...
<template>
  ...
  <div class="campaign-form-container">
    ...
    <VForm ref="refVForm" @submit.prevent="onSubmit">
      <VCol cols="12">
        <AppSelect v-model="selectedPlatform" hide-no-data :item-title="(item) => item.title"
          :item-value="(item) => item.code" :items="platforms" :label="t('platform')" :loading="loadingPlatforms"
      ...
      <VCol cols="12">
        <AppTextInput v-model="form.daily_budget" :append-text="t(user.currency)" :label="$t('campaign_daily_budget')"
          :rules="rules.daily_budget" />
      </VCol>

      <VCol cols="12">
        <AppDateField v-model="dateTimes.startDate" :label="$t('campaign_start_date')" :min="new Date()"
          :placeholder="null" />
      </VCol>
      <VCol cols="12">
      ...
      <VCol cols="12">
        <AppDateField v-model="dateTimes.endDate" :label="$t('campaign_end_date')" :min="new Date()"
          :placeholder="null" />
      </VCol>
      <VCol cols="12">
      ...
      <slot v-if="selectedPlatform == 'googleads'" :data="{
        ...form,
      ...
      <VCol v-if="selectedPlatform != 'googleads'" cols="12">
        <VBtn color="primary" :disabled="loading" :loading="loading" :loading-text="$t('loading')" :text="$t('save')"
