...
<template>
  <VCard>
    <div class="d-flex align-center justify-space-between ga-4 pa-4 mb-4">
      ...
      <div class="filter-container">
        <VDateInput v-model="dateRange" cancel-text="cancel" class="date-range-input" clearable density="comfortable"
          flat hide-details :label="t('select_range_date')" multiple="range" ok-text="ok" persistent-clear
          :prepend-icon="null" prepend-inner-icon="tabler-calendar" variant="solo-filled"
          @click:clear="dateRange = null" />
      </div>
    </div>
    <VDivider />

    <VDataTableServer v-model="selectedItems" v-model:items-per-page="options.itemsPerPage" v-model:page="options.page"
      class="text-no-wrap" :headers="headers" :items="campaigns" :items-length="totalCampaigns" :loading="loading"
      :no-data-text="'no_data_text'" select-strategy="all" show-select @update:options="options = $event">
      <!-- Platform -->
      ...
      <template #item.actions="{ item }">
        ...
        <VTooltip :text="t('edit')">
          <template #activator="{ props: toolTipProps }">
            <VBtn v-bind="toolTipProps" :disabled="item.is_external === true"
              @click="editCampaign(item.id, item.platform)">
              <VIcon icon="tabler-edit" />
            </VBtn>
          </template>
        </VTooltip>
        <VTooltip v-if="statuses.includes(item.status)" :text="item.status === 'Paused' ? t('start_campaign') : t('pause_campaign')
          ">
          <template #activator="{ props: toolTipProps }">
            <VBtn v-if="statuses.includes(item.status)" v-bind="toolTipProps" @click="
      ...
      <template #bottom>
        ...
        <div class="d-flex align-center justify-sm-space-between justify-center flex-wrap gap-3 pa-5 pt-3">
          ...
          <VPagination v-model="options.page" :length="Math.ceil(totalCampaigns / options.itemsPerPage)"
            total-visible="3" />
        </div>
      </template>
    </VDataTableServer>
  ...
  <ConfirmDialog v-model:is-dialog-visible="isDeleteConfirmDialogVisible" :confirmation-question="t('dialog_question')"
    @confirm="deleteConfirmed" />

  <!-- 👉 Activate Confirm Dialog -->
  <ConfirmDialog v-model:is-dialog-visible="isActivateConfirmDialogVisible"
    :confirmation-question="t('dialog_question')" @confirm="activateConfirmed" />

  <!-- 👉 Deactivate Confirm Dialog -->
  <ConfirmDialog v-model:is-dialog-visible="isPauseConfirmDialogVisible" :confirmation-question="t('dialog_question')"
